...
namespace UnityEngine.Rendering.Universal.Internal
{
    ...
    public class PostProcessPass : ScriptableRenderPass
    {
        ...
        Custom.ExponentialHeightFog m_ExponentialHeightFog;
        ...
        public override void Execute(ScriptableRenderContext context, ref RenderingData renderingData)
        {
            ...
            m_ExponentialHeightFog = stack.GetComponent<Custom.ExponentialHeightFog>();
        ...
        void Render(CommandBuffer cmd, ref RenderingData renderingData)
        {
            ...
            bool useExponentialHeightFog = m_ExponentialHeightFog.IsActive();
            ...
            if (useExponentialHeightFog)
            {
                using (new ProfilingScope(cmd, ProfilingSampler.Get(URPProfileId.ExponentialHeightFog)))
                {
                    DoExponentialHeightFog(cameraData.camera, cmd, GetSource(),GetDestination());
                    Swap(ref renderer);
                }
            }
        ...
        #region ExponentialHeightFog
        void DoExponentialHeightFog(Camera camera, CommandBuffer cmd, RenderTargetIdentifier source, RenderTargetIdentifier destination)
        {
            var material = m_Materials.exponentialHeightFog;
            if (material != null)
            {
                float fogDensity = m_ExponentialHeightFog.fogDensity.value;
                float fogOpacity = m_ExponentialHeightFog.fogOpacity.value;
                float fogHeight = m_ExponentialHeightFog.fogHeight.value;
                float heightFalloff = m_ExponentialHeightFog.heightFalloff.value;
                float fogStartDistance = m_ExponentialHeightFog.fogStartDistance.value;
                Color fogColor = m_ExponentialHeightFog.fogColor.value;
                float inscatteringExponent = m_ExponentialHeightFog.inscatteringExponent.value;
                float inscatteringStartDistance = m_ExponentialHeightFog.inscatteringStartDistance.value;

                material.SetFloat("_FogDensity", fogDensity);
                material.SetFloat("_FogOpacity", fogOpacity);
                material.SetFloat("_FogHeight", fogHeight);
                material.SetFloat("_HeightFalloff", heightFalloff);
                material.SetFloat("_FogStartDistance", fogStartDistance);
                material.SetColor("_FogColor", fogColor);
                material.SetFloat("_InscatteringExponent", inscatteringExponent);
                material.SetFloat("_InscatteringStartDistance", inscatteringStartDistance);

                Texture heightNoiseTexture = m_ExponentialHeightFog.HeightNoiseTexture;
                Vector3 heightNoiseScale = m_ExponentialHeightFog.heightNoiseScale.value;
                Vector2 heightNoiseFlowSpeed = m_ExponentialHeightFog.heightNoiseFlowSpeed.value;
                float heightNoisePower = m_ExponentialHeightFog.heightNoisePower.value;

                material.SetTexture("_HeightNoiseTexture", heightNoiseTexture);
                material.SetVector("_HeightNoiseScale", heightNoiseScale);
                material.SetVector("_HeightNoiseFlowSpeed", heightNoiseFlowSpeed);
                material.SetFloat("_HeightNoisePower", heightNoisePower);

                Texture heightColorRampTexture = m_ExponentialHeightFog.HeightColorRampTexture;
                float bottomHeight = m_ExponentialHeightFog.bottomHeight.value;
                float topHeight = m_ExponentialHeightFog.topHeight.value;

                material.SetTexture("_HeightColorRampTexture", heightColorRampTexture);
                material.SetFloat("_BottomHeight", bottomHeight);
                material.SetFloat("_TopHeight", topHeight);
            }
            PostProcessUtils.SetSourceSize(cmd, m_Descriptor);
        ...
        class MaterialLibrary
        {
            ...
            public readonly Material exponentialHeightFog;
            ...
            public MaterialLibrary(PostProcessData data)
            {
                ...
                exponentialHeightFog = Load(data.shaders.exponentialHeightFogPS);
            ...
            internal void Cleanup()
            {
                ...
                CoreUtils.Destroy(exponentialHeightFog);
